...
TriggerEvent('core:getServer', function(obj) Core = obj end)
...
AddEventHandler('nui_doorlock:updateState', function(doorID, locked, src, usedLockpick)
	...
	local xPlayer = Core.Player.getPlayerFromId(playerId)

	if type(doorID) ~= 'number' then
		print(('nui_doorlock: %s (%s) didn\'t send a number! (Sent %s)'):format(xPlayer.getName(), xPlayer.steamId, doorID))
		return
	end

	if type(locked) ~= 'boolean' then
		print(('nui_doorlock: %s (%s) attempted to update invalid state! (Sent %s)'):format(xPlayer.getName(), xPlayer.steamId, locked))
		return
	end

	if not Config.DoorList[doorID] then
		print(('nui_doorlock: %s (%s) attempted to update invalid door! (Sent %s)'):format(xPlayer.getName(), xPlayer.steamId, doorID))
		return
	end
...
RegisterNetEvent('nui_doorlock:getDoorInfo')
AddEventHandler('nui_doorlock:getDoorInfo', function()
	TriggerClientEvent('nui_doorlock:receiveDoorInfo', source, doorInfo)
end)

function IsAuthorized(xPlayer, doorID, locked, usedLockpick)
	local jobName
	jobName = xPlayer.Player.department
